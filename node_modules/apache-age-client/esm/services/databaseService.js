...
export class DatabaseService {
  ...
  async getGraphLabels() {
    ...
    try {
      const queryResult = await graphRepository.execute(getQuery("graph_labels", graphRepository.flavor), [this.getConnectionInfo().graph]);
  ...
  async getGraphLabelCount(labelName, labelKind) {
    ...
    if (labelKind === "v") {
      query = util.format(getQuery("label_count_vertex", graphRepository.flavor), `${this.getConnectionInfo().graph}.${labelName}`);
    } else if (labelKind === "e") {
      query = util.format(getQuery("label_count_edge", graphRepository.flavor), `${this.getConnectionInfo().graph}.${labelName}`);
    }
  ...
  async getNodes() {
    if (!this._graphRepository) throw new Error("not connected");
    const graphRepository = this._graphRepository;
    let queryResult = await graphRepository.execute(util.format(getQuery("meta_nodes", graphRepository.flavor), graphRepository.graph, graphRepository.graph));
    return queryResult.rows;
  }
  ...
  async getEdges() {
    ...
    let queryResult = await graphRepository.execute(util.format(getQuery("meta_edges", graphRepository.flavor), graphRepository.graph, graphRepository.graph));
    return queryResult.rows;
  }
  ...
  async getPropertyKeys() {
    if (!this._graphRepository) throw new Error("not connected");
    let graphRepository = this._graphRepository;
    let queryResult = await graphRepository.execute(getQuery("property_keys", graphRepository.flavor));
    return queryResult.rows;
  }
  ...
  async getRole() {
    if (!this._graphRepository) throw new Error("not connected");
    const graphRepository = this._graphRepository;
    let queryResult = await graphRepository.execute(getQuery("get_role", graphRepository.flavor), [this.getConnectionInfo().user]);
  ...
  async connectDatabase(connectionInfo) {
    ...
    try {
      const client = await GraphRepository.getConnection(graphRepository.getConnectionInfo(), true);
  ...
  async getConnectionStatus() {
    ...
    try {
      const client = await GraphRepository.getConnection(graphRepository.getConnectionInfo());
      if (client.release) client.release();
    } catch (err) {
      return false;
    }
